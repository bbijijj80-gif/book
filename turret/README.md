...
Есть отдельный физический пульт управления (ESP32 + сенсорный дисплей
Nextion, без браузера) — см. [`remote/README.md`](remote/README.md).
...
Скриншоты ниже — настоящий веб-интерфейс проекта (тот же HTML/CSS/JS, что и
в репозитории), запущенный с подставными камерой и моторами вместо реального
Pi. В кадре видео — нарисованная тестовая сцена (силуэты вместо людей), а не
запись с настоящей камеры: так показана разметка (рамки, прицел, «ЗАХВАТ»)
без реальных фотографий людей.

| АВТО, слежение (не взведено) | ВЗВЕДЕНО, цель захвачена |
|---|---|
| ![Режим АВТО: слежение за целью, ещё не захвачена](docs/screenshots/ui-auto-tracking.png) | ![Взведено, цель захвачена — красная подсветка и баннер ЗАХВАТ](docs/screenshots/ui-armed-lock.png) |

| Режим РУЧНОЙ, джойстик | Нет весов детектора (офлайн-откат) |
|---|---|
| ![Режим РУЧНОЙ, джойстик активен и смещён](docs/screenshots/ui-manual-joystick.png) | ![Нет весов детектора — кнопка АВТО отключена, только ручной режим](docs/screenshots/ui-no-detector-fallback.png) |
